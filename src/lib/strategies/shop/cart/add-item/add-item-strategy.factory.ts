...
import { AddServiceItemToCartStrategy } from "./add-service-item-to-cart.strategy";
...
export class AddItemStrategyFactory {
    ...
    private serviceProductStrategy: AddServiceItemToCartStrategy;
    ...
    constructor(shopModel: ReturnType<typeof useShopModel>) {
        ...
        this.serviceProductStrategy = new AddServiceItemToCartStrategy(shopModel);
    }
    ...
    getStrategy(item: InterfaceInventoryItem): AddItemToCartStrategy {
        if (this.serviceProductStrategy.canHandle(item)) {
            return this.serviceProductStrategy;
        }
